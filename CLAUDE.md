...
- **`noindex` forces both scores to 0; `nosnippet` is flagged as a P0 recommendation.** A
  ...
  snippet, AI features included). Checks both the `<meta name="robots">`/`<meta name="googlebot">`
  form and the equivalent `X-Robots-Tag` HTTP response header (as of 2026-08-21; previously only
  the meta-tag form was checked, a documented gap): the two forms block identically in the real
  world, so a page relying only on the header would otherwise be scored as if it were indexable
  when it isn't.
- **Informational-only signals, shown but never scored, same treatment as llms.txt**: the
  `<meta name="viewport">` tag (real mobile-friendliness needs rendering/performance measurement
  this single-fetch audit doesn't do) and a `Content-Signal` directive in robots.txt (a brand-new,
  2025, Cloudflare-proposed convention, not a Google-recognized directive and not confirmed to be
  honored by any major crawler yet: https://blog.cloudflare.com/content-signals-policy/). Each has
  its own `why` explanation in `src/lib/category-content.ts`; don't assume they share llms.txt's
  specific reason for being unscored, since they don't.
...
## Analytics & consent
- **Implemented** (2026-08-24), reusing this project's `ANALYTICS-CONSENT-SETUP.md` runbook and
  the same setup as `dahiana.work`/camping-jacobo-tayrona exactly: GTM container `GTM-KFBP33SQ`,
  Consent Mode v2 with the real `gtag()` shim (not a hand-rolled `dataLayer.push(array)`, which
  silently fails to gate consent — see the runbook §5 for why), opt-in by default, equal-weight
  Accept/Reject, cookie auto-clear on reject.
- Consent banner UI: `vanilla-cookieconsent` (pinned exact, `3.1.0`, zero dependencies — same
  version used on the reference sites). Two categories only: Necessary (just `cc_cookie` itself,
  read-only/always-on) and Analytics (`_ga`/`_ga_*`, gates `analytics_storage`). No Marketing
  category: this project has no ads/remarketing tags, so one wasn't added speculatively.
- Files: `src/layouts/BaseLayout.astro` (consent-default + GTM loader scripts, noscript iframe),
  `src/components/CookieConsent.astro` (banner/preferences panel, styled with this project's own
  tokens), `src/pages/cookies.astro` (real cookie table + "Manage cookies" button reopening the
  preferences panel), `src/consts.ts` (`GTM_ID`).
- The consent-default script must be the literal first script in `<head>`, before the GTM
  loader; verified against rendered page source in both dev and a production build (not just
  dev) — re-verify the same way against the deployed site after every change here.
- The GA4 "Google tag" itself still needs configuring inside the `GTM-KFBP33SQ` container
  (tagmanager.google.com → Tags → Google tag → Measurement ID → All Pages trigger → Publish) —
  that's a console step, not something in this codebase.
