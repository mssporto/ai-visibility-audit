...
components generate directly and depend on for correct rendering — `.alignment-start/-center/
...
score-card row, then a single-column results panel (`--dashboard-max-width: 48rem`, also the nav
and footer's inner max-width).
...
9. **Data rows** — bespoke: horizontal flex, a status icon (✓ green / ! red) plus category label
   on the left, score + "Needs attention" text on the right; `--border-width` `--border` dividers.
